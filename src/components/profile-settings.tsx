...
export function ProfileSettings() {
  ...
  return <form onSubmit={submit} className="flex flex-col gap-5"><Card><CardHeader><CardTitle>Profile</CardTitle><CardDescription>Your name and avatar color.</CardDescription></CardHeader><CardContent><FieldGroup><Field><FieldLabel>Avatar</FieldLabel><div className="flex items-center gap-4"><UserAvatar name={name} color={color} className="size-10" /><UserColorPicker value={color} onValueChange={setColor} /></div></Field><Field><FieldLabel htmlFor="display-name">Display name</FieldLabel><Input id="display-name" value={name} onChange={(event) => setName(event.target.value)} required /></Field><Field><FieldLabel>Email</FieldLabel><Input value={user.email} disabled /><FieldDescription>Email changes are managed by an admin.</FieldDescription></Field></FieldGroup></CardContent></Card>
