...
export type LogSummary = { id: string; workspaceId: string; authorId: string; authorName: string; day: string; title: string | null; body: string; createdAt: string; updatedAt: string; tags: string[]; mirrorDirty: boolean; posX: number | null; posY: number | null; width: number | null; height: number | null; zIndex: number; isLocked: boolean };
...
export const api = {
  ...
  updateLog: async (workspaceId: string, logId: string, values: { title?: string | null; body?: string; isLocked?: boolean }) => readJson<LogDetail>(await apiClient.api.workspaces[":workspaceId"].logs[":logId"].$patch({ param: { workspaceId, logId }, json: values })),
